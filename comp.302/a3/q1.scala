...
object WML extends App {
  ...
  def prettyPrint(rootNode: ASTNode):String = {
    rootNode match {
      //
      case ASTProgram(outer) => outer.foldLeft(""){ (first, second) => first + prettyPrint(second)}
      case ASTInvoke(name, targs) => "{{" + prettyPrint(name).trim + prettyPrint(targs).trim + "}}"
      case ASTTargs(targs) => targs.foldLeft(""){ (first, second) => "|" + first + prettyPrint(second).trim}
      case ASTItext(itext) => itext.foldLeft(""){(first, second) => first + prettyPrint(second)}
      case ASTTvar(name, opt) => "{{{" + name.trim + prettyPrint(opt).trim + "}}}"
      case ASTTdef(name, dparams, body) => "{'" + prettyPrint(name).trim + prettyPrint(dparams).trim + "||" + prettyPrint(body) + "'}"
      case ASTDparams(dparams) => dparams.foldLeft(""){(first, second) => "|" + first + prettyPrint(second)}
      case ASTDtext(k, dtext) => dtext.foldLeft(""){(first, next) => first + prettyPrint(next)}
      case ASTText(k, s) => s //easiest one
    }
  }
  ...
  if (result.successful) {
    println(prettyPrint(result.get))
  } else {
